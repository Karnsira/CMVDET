import os
from lib.utils_cls import load_database, generate_patches_cmv, generate_patches_fp, generate_patches_hard, generate_patches_bg, train_val_split
import argparse
...
cnt = generate_patches_fp(size=size,
                          res_path=det_res,
                          conf_thres=0.2,
                          WSI_path=WSI_path, 
                          db_path=db_path,
...
train_val_split(annotations_path=save_to+'annotations.csv', 
                slide_val=val_slide, 
                num_train=(len(load_database(db_config=DatabaseConfig(path=db_path, slide_list=train_slide, reviewed=reviewed)))*repeat)-(len(load_database(DatabaseConfig(path=db_path, slide_list=val_slide, reviewed=reviewed)))*repeat), 
