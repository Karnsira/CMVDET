...
def load_database(db_config : DatabaseConfig):
    
    path = db_config.path
    slide_list = db_config.slide_list
    class_list = db_config.class_list
    fold = db_config.fold
    manual = db_config.manual
    reviewed = db_config.reviewed
...
def generate_patches_cmv(size:int, WSI_path:str, save_patches_to:str, save_patches_anno_to:str, db_config : DatabaseConfig, repeat=10):
    
    anno_df = load_database(db_config)
    ...
    for filename in tqdm(class_df['filename'].unique(), desc='Generating CMV patches'):
        ...
        for i in range(len(slide_df)):
            ...
            for n in range(repeat):
                x_move = randrange(-int(size/2 - anno_width/2), int(size/2 - anno_width/2))
                y_move = randrange(-int(size/2 - anno_height/2), int(size/2 - anno_height/2))
                ...
                # find other cmv in the same patch
                patch_data = find_anno_in_patch(slide_df, filename, patchname, size, (patch_x_origin, patch_y_origin), (x_move, y_move), (x_change, y_change))

                for row in patch_data:
                    patches_data.append(row)

    patches_df = pd.DataFrame(patches_data, columns=['slidename','filename', 'annoID', 'width',
                                                     'height','x_min','y_min','x_max','y_max','class'])
    patches_df.to_csv(save_patches_anno_to+'/annotations.csv', index=False)
    #print('***Total',cnt,'patches***')
    
    
def generate_patches_bg(size:int, WSI_path:str, db_config : DatabaseConfig, save_patches_to:str, number_per_wsi:int, color_thres:int, start_idx:int=0, fold=[1,2]):
...
def generate_patches_hard(size:int, WSI_path:str, save_patches_to:str, start_idx:int=0, db_config = DatabaseConfig(class_list=[0,5,6], fold=[1,2], manual=True), repeat=10):
